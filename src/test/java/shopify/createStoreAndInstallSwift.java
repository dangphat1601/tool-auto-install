...
public class createStoreAndInstallSwift extends AbstractTest {
    ...
    public void TC01_CreateShopifyStore() throws IOException {
        ...
        log.info("Click create new Oberlo");
        oberloPage.inputToEmailObelo("email oberlo", email);
        log.info("email Oberlo:" + email);
        ...
        if (isPageNotResult5) {
            shopifyPage.searchAppInShopifyAppStore();
            shopifyPage.inputKeywordWhenNoResult(swiftPage.getRandomKeyword());
        }

        log.info("Step 33: Select Swift app");
        swiftPage.clickToURLAppSwift();

        log.info("Step 34: Add Swift app to store");
        ...
        log.info("Step 36: Add Swift app to store");
